            ...
            if self.Data_Column_Entry.get() == 'Row Index':
                ...
                for i in row_index:
                    ...
                    try:
                        ...
                        #t = threading.Thread(target = self.produce_word, args = (context, file_, path_, Names))    
                        #t.setDaemon(True)
                        #t.start()
                        #print(t)
                        threading.Thread(target = self.produce_word, args = (context,file_,path_,Names)).start()  
                        #t.setDaemon(True)
                        #t.start()
                        #eval('t%s t.setDaemon(True)' %i)
                        #eval('t%s.start()' %i)
                        #eval('print(t%s' %i)
                    except Exception as e:
                        self.write_log_to_Text("INFO: %s" %e)
            ...
            if self.Data_Column_Entry.get() == 'Row Index':
                ...
                for i in row_index:
                    ...
                    try:
                        ...
                        raise e
                    try:
                        eval('t%s = threading.Thread(target = self.produce_pdf, args = (context, file_, path_, Names))' % i)    
                        #t.setDaemon(True)
                        #t.start()
                        #eval('t%s t.setDaemon(True)' %i)
                        eval('t%s.start()' %i)
