    ...
    def get_data_from_excel(self, sheet_index = 0):
        ...
        for index in data.columns:
            ...
            try:
                ...
                pass
            try:
                data[index] = data[index].apply(cleanstr)
            except:
                pass
        percent = list(filter(lambda x: '百分数' in x, data.columns))
        if len(percent)>=1:
            for col in percent:
...
def gui_start():
    ...
    MapFiles_GUI_ini = MapFiles_GUI(init_window)
    # 设置根窗口默认属性
    MapFiles_GUI_ini.set_init_window()
